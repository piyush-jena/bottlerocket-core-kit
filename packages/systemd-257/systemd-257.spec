...
Patch9018: 9018-do-not-build-32-bit-ia32-sd-boot.patch
...
Requires: %{name}
Provides: %{_cross_os}bootloader
Provides: %{_cross_os}uki-stub
Provides: %{_cross_os}uki-bootloader
...
CONFIGURE_OPTS=(
 ...
 # SBAT revocation metadata embedded in the sd-boot/sd-stub .sbat PE section.
 # sbat-distro/-summary/-url MUST be set explicitly: this is a cross build
 # (the SDK's cross_meson macro passes --cross-file), so meson.build's
 # os-release autodetection branch (src/boot/meson.build,
 # `not meson.is_cross_build()`) never runs, and leaving sbat-distro=auto
 # (the default) hits the cross-build guard that disables SBAT entirely.
 ...
 -Dsbat-distro-url='https://github.com/bottlerocket-os/bottlerocket/blob/develop/SECURITY.md'
