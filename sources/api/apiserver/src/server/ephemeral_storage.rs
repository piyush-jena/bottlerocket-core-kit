...
fn encrypt_ephemeral_device(device: &str) -> Result<String> {
    ...
    if is_ephemeral_encryption_keys_enabled {
        // A single combined step opens the device as a dm-crypt mapper using a per-boot random
        // key. There is no format step and thus no need to check whether the device is already
