...
- `encrypt-and-attach block-device <path> <key-id>` - Encrypt (plain mode) and attach a block device in one step, using the key named by `<key-id>`
...
- `resize block-device <path> <key-id>` - Resize a block-device mapper to the grown device
...
/// Encrypt (plain mode) and attach a resource in one step
...
#[derive(FromArgs)]
#[argh(subcommand, name = "block-device")]
/// Encrypt (plain mode) and attach a block device in one step
...
#[derive(FromArgs)]
#[argh(subcommand, name = "block-device")]
/// Resize an encrypted block device (LUKS2 mapper; the key-id names its TPM-sealed key)
